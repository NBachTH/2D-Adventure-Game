...
public class Monster extends Entity {
    ...
    public void attackPlayer() {
        ...
        if (touchPlayer == true) {
            if (gp.player.invincible == false) {
                ...
                if (damage > 0) {
                    gp.player.HP = gp.player.HP - damage;
                } else {
                    gp.player.HP--;
                }
                gp.playSE(3);
    ...
    public void dropItem() {
        ...
        for (int i = 0; i < gp.item.length; i++) {
            if (gp.item[i] == null) {
                switch (a) {
                    case 1 -> item = new OBJ_Potion_Blue(gp, x / gp.tileSizeX, y / gp.tileSizeY);
                    case 2 -> item = new OBJ_Potion_Red(gp, x / gp.tileSizeX, y / gp.tileSizeY);
                    case 3, 4, 5, 6 -> item = new OBJ_Heart(gp, x / gp.tileSizeX, y / gp.tileSizeY);
                    case 7, 8, 9, 10 -> item = new OBJ_HeartHalf(gp, x / gp.tileSizeX, y / gp.tileSizeY);
                }
                if (item != null) {
                    gp.item[i] = item;
